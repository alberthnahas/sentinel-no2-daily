...
def process_box(connection, start_date, end_date, box_name, spatial_extent):
    ...
    output_filename = f"NO2_{box_name}_{start_date.replace('-', '')}.nc"
...
def merge_boxes(date_str, box_files):
    ...
    merged_filename = f"NO2_Indonesia_Daily_{date_str.replace('-', '')}_merged.nc"
...
def process_and_save_data(ds, date_str, method="original"):
    ...
    final_filename = f"{base_filename}_{method}.nc"
...
def main():
    ...
    if len(sys.argv) != 2:
        print("Usage: python xx-sentinel-no2-date.py YYYY-MM-DD")
        print("Example: python xx-sentinel-no2-date.py 2023-12-15")
        sys.exit(1)
    ...
    print(f"📅 Processing SENTINEL-5P NO2 data for date: {date_str}")
